...
@interface IDMPhotoBrowser () {
	...
	// Toolbar
	UIToolbar *_toolbar;
	UIBarButtonItem *_previousButton, *_nextButton, *_actionButton, *_shareButton;
...
- (void)viewDidLoad {
    ...
    // Close Button
    if(!_doneButtonImage) {
        _doneButton = [[UIBarButtonItem alloc] initWithCustomView:[self customToolbarButtonImage:[UIImage imageNamed:@"IDMPhotoBrowser.bundle/images/IDMPhotoBrowser_arrowLeft.png"]
                                                                                   imageSelected:[UIImage imageNamed:@"IDMPhotoBrowser.bundle/images/IDMPhotoBrowser_arrowLeftSelected.png"]
                                                                                          action:@selector(doneButtonPressed:)]];
    }
    else {
        _doneButton = [[UIBarButtonItem alloc] initWithCustomView:[self customToolbarButtonImage:_doneButtonImage
                                                                                    imageSelected:_doneButtonImage
                                                                                           action:@selector(doneButtonPressed:)]];
    }
    ...
    _counterLabel.font = [UIFont systemFontOfSize:16];
    ...
    // Action Button
    _actionButton = [[UIBarButtonItem alloc] initWithCustomView:[self customToolbarButtonImage:[UIImage imageNamed:@"download"]
                                                                                 imageSelected:[UIImage imageNamed:@"download"]
                                                                                        action:@selector(actionButtonPressed:)]];
    
    _actionButton.tintColor = [UIColor whiteColor];
    
    //share Button
    _shareButton = [[UIBarButtonItem alloc] initWithCustomView:[self customToolbarButtonImage:[UIImage imageNamed:@"tb_share"]
                                                                               imageSelected:[UIImage imageNamed:@"tb_share"]
...
- (CGRect)frameForDoneButtonAtOrientation:(UIInterfaceOrientation)orientation {
    ...
    return CGRectMake(20, screenHeight - 35, 26, 26);
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView  {
    ...
    if (_currentPageIndex != previousCurrentPage) {
        if(self.displayLastView && _currentPageIndex == [self numberOfPhotos]){
            if(!isLoadLastView){
                CGRect lastViewRect = [self frameForPageAtIndex:[self numberOfPhotos]];
                _lastView.frame = lastViewRect;
                [_pagingScrollView addSubview:_lastView];
                isLoadLastView = YES;
            }
            //隐藏toobar
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
	...
	[self setControlsHidden:controlsShow animated:YES permanent:YES];
}
...
- (void)updateToolbar {
	...
	if ([self numberOfPhotos] > 1) {
		_counterLabel.text = [NSString stringWithFormat:@"%u/%lu", _currentPageIndex+1, (unsigned long)[self numberOfPhotos]];
	} else {
...
- (void)toggleControls    {
    controlsShow = ![self areControlsHidden];
...
- (void)image: (UIImage *) image didFinishSavingWithError: (NSError *) error contextInfo: (void *) contextInfo {
    if ([_delegate respondsToSelector:@selector(savePhoto:)]) {
        [_delegate savePhoto:error != NULL];
    }
}
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        {
            UIImage *savedImage = [photo underlyingImage];
...
-(void) shareButtonPressed:(id) sender{
    if([_delegate respondsToSelector:@selector(sharePhote:)]){
        [_delegate sharePhote:_userInfo];
    }
}
