...
+ (NSArray *)photosWithURLsAndCaption:(NSArray *)urlsArray {
    NSMutableArray *photos = [NSMutableArray arrayWithCapacity:urlsArray.count];
    
    for (NSDictionary *dictionary in urlsArray) {
        id url = [dictionary objectForKey:@"url"];
        if ([url isKindOfClass:[NSURL class]]) {
            IDMPhoto *photo = [IDMPhoto photoWithURL:url];
            photo.caption = [dictionary objectForKey:@"caption"];
            photo.content = [dictionary objectForKey:@"content"];
            [photos addObject:photo];
        }
        else if ([url isKindOfClass:[NSString class]]) {
            IDMPhoto *photo = [IDMPhoto photoWithURL:[NSURL URLWithString:url]];
